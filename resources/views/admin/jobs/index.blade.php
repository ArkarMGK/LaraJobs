        ...
        <div class="col-lg-12 mx-auto">
            <div class="card">
                <div class="card-header d-flex align-items-center">
                    <legend class="text-center">List of Active Jobs</legend>
                    <form action="{{ route('admin#jobList') }}" method="GET" class="d-flex">
                        <input type="text" name="search" class="form-control" placeholder="Search"
                            value="{{ request('search') }}" type="submit">
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-4 jobStatus">
                                            <form action="{{ route('admin#hiredJob', $job) }}" method="POST">
                                                @csrf
                                                <select class="changeStatus form-control w-100 mt-4 "
                                                    name="jobCurrentStatus">
                                                    <option value="1" @if ($job->available) selected @endif
                                                        style="color: blue">
                                                        Vacant
                                                    </option>
                                                    <option value="0" @if ($job->available == 0) selected @endif
                                                        style="color: red">
                                                        Hired
                                                    </option>
                                                </select>
                                                <input type="submit" value="Approved"
                                                    class="submitBtn btn btn-danger mt-2 w-100" disabled>
                                            </form>
                                        </div>
                                    </div>
                                </div>
        ...
        $(document).ready(function() {
            ...
            $('.changeStatus').change(function() {
                ...
                if ($current == '0') {
                    ...
                    $($parentNode).find('.submitBtn').removeAttr("disabled");
                } else if ($current == '1') {
                    $($parentNode).find('.changeStatus').css('color', 'blue');
                    $($parentNode).find('.submitBtn').attr("disabled", true);
                }
